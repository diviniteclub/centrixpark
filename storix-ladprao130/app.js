const defaultUnits = [
  ...
  { id: "LP5", side: "right", type: "Standard Unit", size: "5.8 x 17 ม.", area: "124 ตร.ม.", price: 40000, status: "reserved", note: "มีผู้เช่าแล้ว" },
...
const storageKey = "storix-ladprao130-units-v3";
...
function formatPrice(price) {
  return price > 0 ? `${money.format(price)} บาท/เดือน` : "สอบถามราคา";
}
...
function renderMap() {
  ...
  units.forEach((unit) => {
    ...
    button.innerHTML = `
      ...
      <span>
        <strong>${formatPrice(unit.price)}</strong>
...
function renderSelected() {
  ...
  const lineMessage = `สนใจสอบถามรายละเอียด Storix Warehouse ลาดพร้าว 130 ยูนิต ${unit.id} ราคา ${priceText} ขนาด ${unit.size} พื้นที่ ${unit.area}`;
  card.innerHTML = `
